...
class _netE(nn.Module):
    def __init__(self, nc, nz, ngf, kernel=2, padding=1, img_width=64, img_height=64, quant_levels=None, do_comp=False, ncenc=8, nresenc=0, detenc=False, noisedelta=0.5, bnz=False, ngpu=1 , rmvquant = False):
        ...
        self.nfmodelz = math.ceil(nz / ((img_height//32) * (img_width//32))) + ncenc #ngf * 8
        self.ncenc = ncenc
        ...
        model_down_list = [
            ...
            nn.ReLU(True),
            # one more layer 
            nn.Conv2d(ngf * 8, ngf * 8, kernel, 2, padding, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True)
        ]
        ...
        model_z_list += [nn.Conv2d(ngf * 8, nz, (img_height//32, img_width//32), 1, 0, bias=False)]
...
class _netG(nn.Module):
    def __init__(self, nc, nz, ngf, kernel=2, padding=1, output_padding=0, img_width=64, img_height=64, nresdec=0, ngpu=1):
        ...
        main_list = [nn.ConvTranspose2d(nz, ngf * 8, (img_height//32, img_width//32), 1, 0, bias=False),
                nn.BatchNorm2d(ngf * 8),
                nn.ReLU(True)]
        ...
        main_list += [
            ...
            nn.ConvTranspose2d(ngf * 8, ngf * 8, kernel, 2, padding, output_padding, bias=False),
            nn.BatchNorm2d(ngf * 8),
            nn.ReLU(True),
